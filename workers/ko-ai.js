...
function validateBriefingCompliance(stratId, outputText) {
  const missing = [];
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (!expert_mode && action === 'ki_briefing' && _OPTIONS_STRATEGY_IDS.indexOf(strategy) !== -1) {
        const initialCheck = validateBriefingCompliance(strategy, text);
        if (initialCheck.status === 'FAIL') {
          ...
          try {
            const repairPrompt = buildRepairPrompt(strategy, initialCheck.missing, text);
            ...
            const repairCheck = validateBriefingCompliance(strategy, repairText);
