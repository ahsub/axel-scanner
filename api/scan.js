...
export default async function handler(req) {
  ...
  if (mode === 'movers') return getMostActive(req);
  if (mode === 'afterhours') return getAfterHours(req);
...
async function getMostActive(req) {
  try {
    const urls = [
      'https://query1.finance.yahoo.com/v1/finance/screener/predefined/saved?formatted=false&scrIds=most_actives&count=12&start=0&region=US&lang=en-US',
      'https://query2.finance.yahoo.com/v1/finance/screener/predefined/saved?formatted=false&scrIds=most_actives&count=12&start=0&region=US&lang=en-US',
    ];
    let data = null;
    for (const url of urls) {
      try {
        ...
        if (quotes && quotes.length > 0) { data = quotes; break; }
      } catch(e) { continue; }
    }
    if (!data) throw new Error('No most active data');
    const result = data.slice(0, 10).map(q => ({
      symbol: q.symbol,
      ...
      volume: formatVol(q.regularMarketVolume),
      avgVol: formatVol(q.averageDailyVolume3Month),
      volRatio: q.averageDailyVolume3Month > 0
        ? Math.round((q.regularMarketVolume / q.averageDailyVolume3Month) * 100)
        : null,
      mktCap: formatCap(q.marketCap),
    }));
    return ok({ mode: 'movers', items: result, ts: new Date().toISOString() });
  } catch(e) {
    return err('Most active: ' + e.message);
  }
}
 
async function getAfterHours(req) {
  try {
    const watchlist = ['AAPL','MSFT','NVDA','GOOGL','META','AMZN','TSLA','AMD','AVGO','MRVL',
                       'VRT','LRCX','TSM','ARM','CRDO'];
    const syms = watchlist.join(',');
    const urls = [
      `https://query1.finance.yahoo.com/v8/finance/quote?symbols=${syms}&formatted=false&fields=symbol,shortName,regularMarketPrice,regularMarketChangePercent,preMarketPrice,preMarketChangePercent,postMarketPrice,postMarketChangePercent,regularMarketVolume`,
      `https://query2.finance.yahoo.com/v8/finance/quote?symbols=${syms}&formatted=false&fields=symbol,shortName,regularMarketPrice,regularMarketChangePercent,preMarketPrice,preMarketChangePercent,postMarketPrice,postMarketChangePercent,regularMarketVolume`,
    ];
    let quotes = null;
    for (const url of urls) {
      try {
        ...
        if (q && q.length > 0) { quotes = q; break; } else { lastError = 'Empty result from ' + host; }
      } catch(e) { continue; }
    }
    if (!quotes) throw new Error('No after hours data: ' + lastError);
    const now = new Date();
    ...
    const result = quotes.map(q => {
      ...
      return {
        ...
        regularPrice: q.regularMarketPrice?.toFixed(2) || '—',
        regularChgPct: q.regularMarketChangePercent?.toFixed(2) || '0',
        ahPrice: ahPrice?.toFixed(2) || null,
        ahChgPct: ahChg?.toFixed(2) || null,
        volume: formatVol(q.regularMarketVolume),
      };
    }).filter(q => q.ahPrice !== null)
      .sort((a,b) => Math.abs(parseFloat(b.ahChgPct||0)) - Math.abs(parseFloat(a.ahChgPct||0)));
    return ok({ mode: 'afterhours', items: result, isPreMarket, ts: new Date().toISOString() });
  } catch(e) {
    return err('After hours: ' + e.message);
  }
}
